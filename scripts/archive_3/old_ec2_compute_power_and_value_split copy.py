...
def main():
    ...
    price_col = find_column(df, ["On Demand Filled", "price_usd_per_hour", "On Demand Numeric", "On Demand"], required=False)
    instance_type_col = find_column(df, ["API Name", "Instance type", "Instance Type", "instance_type", "Name"], required=True)
    ...
    storage_details = df[storage_col].apply(parse_instance_storage).apply(pd.Series) if storage_col else pd.DataFrame(index=df.index)
    ...
    df["ssd_idle_w_total"] = df["estimated_drive_count"].fillna(0) * pd.to_numeric(df["idle_w"], errors="coerce").fillna(0)
    df["ssd_active_w_total"] = df["estimated_drive_count"].fillna(0) * pd.to_numeric(df["active_w"], errors="coerce").fillna(0)
    ...
    df["gpu_mapping_status"] = df["gpu_tdp_w_per_gpu"].apply(lambda x: "resolved" if pd.notna(x) else "missing")
    ...
    for case_name, cfg in SCENARIO_CONFIG.items():
        ...
        out["total_cpu_power_w"] = out["vcpu_count"].fillna(0) * out["cpu_tdp_w_per_vcpu"].fillna(0) * cpu_util
        out["total_gpu_power_w"] = out["gpu_count"].fillna(0) * out["gpu_tdp_w_per_gpu"].fillna(0) * gpu_util
        out["total_ram_power_w"] = out["memory_gib"].fillna(0).apply(lambda x: scenario_ram_w(x, ram_state))
        out["total_ssd_power_w"] = out.apply(lambda r: scenario_ssd_w(r["ssd_idle_w_total"], r["ssd_active_w_total"], ssd_state), axis=1)
        out["other_components_power_w"] = OTHER_COMPONENTS_FACTOR * ( 
            out["total_gpu_power_w"] + out["total_cpu_power_w"] + out["total_ram_power_w"] + out["total_ssd_power_w"] 
        )
        out["total_power_w"] = (
        ...
        valid = out["price_usd_per_hour"].notna() & out["total_power_mw"].notna() & (out["total_power_mw"] > 0)
        out.loc[valid, "voll_usd_per_mwh"] = out.loc[valid, "price_usd_per_hour"] / out.loc[valid, "total_power_mw"]
    ...
    print(f"Rows with VOLL computed: {long_df['voll_usd_per_mwh'].notna().sum()}")
