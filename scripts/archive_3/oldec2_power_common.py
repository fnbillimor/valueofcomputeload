...
RAM_IDLE_W_PER_GB = 0.19
RAM_ACTIVE_W_PER_GB = 0.54
...
CPU_LOOKUP = {
    ...
    "Intel Xeon E5-2686 v4 (Broadwell)": {
        ...
        "source_note": "AWS-associated custom part; direct vendor-like proxy value.",
    },
    "Intel Xeon E5-2670 (Sandy Bridge)": {
...
def select_ssd_proxy(storage_total_gb: float, drive_count_explicit: Any, is_local_ssd: bool) -> Dict[str, Any]:
    ...
    if per_drive_gb > 2500:
        proxy = SSD_PROXY_CATALOG["medium"].copy()
    if per_drive_gb > 5000:
        proxy = SSD_PROXY_CATALOG["large"].copy()
    if per_drive_gb > 10000:
            proxy = SSD_PROXY_CATALOG["xlarge"].copy()
    else:
        proxy = SSD_PROXY_CATALOG["small"].copy()

    if not drive_count:
        proxy_capacity = float(proxy["proxy_capacity_gb"])
        drive_count = max(1, int(math.ceil(storage_total_gb / proxy_capacity)))
        per_drive_gb = storage_total_gb / drive_count
